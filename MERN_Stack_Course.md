...
So *express* is going to be what we use for the the web server it's going to help us make the web server cores stands for cross-origin resource sharing and it allows ajax requests to skip the same origin policy and access resources from remote hosts the course package provides an express middleware that can enable cores with different options basically it's going to make it so we can make the right connections on our network that we need to make without that we could have some errors and then the mongodb dependency allows us to interact with our mongodb database the emv dependency loads environmental
10:16
