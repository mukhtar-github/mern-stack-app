...
Okay I'm just checking over on the browser to see how it's looking, and I see an error. So it's obviously, it's really good to check things and test things frequently to see if you have an error. And then it reminded me of something that we forgot to do, it says *you should not use a link outside a router*. So let's go back over here and so we had updated the App.js file but there's a file that we need to update also that's actually even loaded right before App.js which is the index.js and this is really what kind of loads everything else and react
