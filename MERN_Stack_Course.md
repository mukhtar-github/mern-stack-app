...
I'll go to *restaurants.route.js*, and we will need to make it so it uses the *controller file* we are about to create. So the *controller file* is going to be what the *route* uses. So I'm going to *import RestaurantsCtrl from "./restaurants.controller.js"*. And now, instead of getting *(req, res) => res.send("Hello World"))*, I'm going to do -- *get(RestaurantsCtrl.apiGetRestaurants)*, and that's something we still have to create.
