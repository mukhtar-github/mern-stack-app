...
So inside this root folder, I'm going to create a new folder called *backend*, and then I'll switch into it. Okay now that I'm in this folder, I'm going to create a *package.json* file inside this folder by running *npm init -y*. Okay we've initiated our *package.json* file, since we're going to be using *node* here. And now we're going to install a few dependencies. So we'll do *npm install*, we're going to do *express, cores, mongodb, and dotenv*. So let me tell you a little bit about these. (npm install express body-parser cors mongoose mongodb dotenv)
...
So after rendering, it's going to *retrieveRestaurants and retrieveCuisines*. And these are functions, we can look at those functions right now. So as you can see at the top, in the *retrieveRestaurants function, we're doing the RestaurantDataService.getAll()*, and that's something that we just created in the *RestaurantDataService*. And then it's going to *log the data, but more importantly, it's going to set the restaurants to be -- setRestaurants(response.data.restaurants)*. And so this is going to go into the *restaurants state*.
...
And we're also going to *retrieveCuisines*. Very similar we're going to use the *data service to get cuisines -- RestaurantDataService.getCuisines()*. And we are going to *log them, and then we're going to set the cuisines -- setCuisines(["All Cuisines"].concat(response.data)), so this is going to be in the drop down menu*. So instead of just putting the *response.data into the cuisines variable*, first we're going to start with an *"All Cuisines" element, and then we're going to concat to this array the rest of the data*.
...
So the *first element in the data is going to be "All Cuisines"*. So that's going to be the *first item in the drop-down menu, if you don't want to select a specific cuisine, but you want to show all the cuisines*. So if we go a bit up of the file as seen above, you'll see soon that there's going to be a *form at the top of the page, and people can search for by name -- onChangeSearchName, zipcode -- onChangeSearchZip, or cuisine -- onChangeSearchCuisine*.
...
So if someone *searches for a name, a zipcode, or a cuisine, when someone types into the search box, we're going to take the value of the search box -- e.target.value, and we're going to set the name -- setSearchName(searchName), or set the zip or set the cuisine to whatever thing that the person typed in, in the case of the cuisine if they selected it*. Up here's a function to *refresh the list of restaurants, if that's needed.* One of the times is, if someone *searches for All Cuisines, it will retrieve all the restaurants.*
...
Okay, if someone tries to *find something, the find function is actually going to be called from the findByName, findByZip, and findByCuisine functions.* So someone's going to type in the *name* that they're going to *search for,* and they're going to *click the button to search.* Once the *button is clicked, then we're going to call the findByName function.* And they want to *call the find and pass in the searchName and the name -- find(searchName, "name").*

If they're *finding by zip -- findByZip, we're going to pass in the searchZip and "zipcode" -- find(searchZip, "zipcode"). If they're finding by cuisine -- findByCuisine, we're going to pass in the searchCuisine that they selected, and the word "cuisine", or if they have selected "All Cuisines", we're just gonna refresh the list -- refreshList().* So then it goes to the *find function.* The *find function is going to pass in the query and the by -- find = (query, by).* So for example, here *the query is searchName and the by is the name -- find(searchName, "name").* So we're going to *call the RestaurantDataService.find(query, by) with the query and the by.*
...
If we go back to the *RestaurantDataService class file, we can see the ${query} and the ${by}.* So *${by} would be name, so let's say the name we're looking for, is for all the restaurants that have the word food in their name. So ${by} would say name, and then ${query} would say food. Or ${by} could say zipcode and then ${query} could be the number of the zipcode.* So after we *get the result of the find, we're going to console log the data -- console.log(response.data).* And then we're going to *set restaurants to be whatever comes back from the backend server -- setRestaurants(response.data.restaurants).* Okay those are all the functions.

So let's go really quick over the *html* that we're creating. Now we're making heavy use of *bootstrap, so most of these classes -- (div className="row pb-1") are directly from bootstrap to help style things.* And we can see that we have *an input group -- (div className="input-group col-lg-4"), because at the top of the page, we're going to have three input boxes or three ways that people can search.*
...
Then we have this next section which is the *search by zip -- placeholder="Search by zip", is going to be just like the one we just saw for name, but it's going to be for the zip. So someone can onChangeSearchZip and then click the button, it's going to findByZip.*
...
And then the final one is going to be a little different, it's going to be a *drop-down menu for the cuisines.* We're going to see all this in just a second on the page. So we're going to *select -- onChange={onChangeSearchCuisine}.* So *onChange it's going to do onChangeSearchCuisine, it's going to set the cuisine variable to the cuisine that the person selected.* And to *get the cuisines in the list, we're going to use this react function -- cuisines.map(cuisine => {}.*

Remember, *cuisines is the list of cuisines, and we're going to map it, and for each cuisine in that array, we're going to return an option for the select box, and the value is going to be the cuisine.* And then the reason why it's *cuisine.substr(0, 20), because some cuisines are very very long, and we want to make sure the search box isn't too long.* So we're going to just have it, just the first 21 characters. Just the first little bit of the cuisine. So people are going to *search for the cuisine, and they'll click the button it'll find my cuisine -- onClick={findByCuisine}.*
