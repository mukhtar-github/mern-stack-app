...
So it's going to do a *get request -- http.get(`restaurants?${by}=${query}&page=${page}`), and this is what it's going to add to the end of the baseURL*. And it's going to be *like zip code, by, equals, and then the query is going to be what the zip code is and the page number*. *createReview* is just going to do a *post request -- http.post("/review-new", data)*. This */review-new with the data*. *updateReview put request with the data -- http.put("/review-edit", data)*.

*deleteReview* but you're going to have the id on the end or get cuisines okay now we can start creating our component our restaurant list component first of all let's import a little more things.
