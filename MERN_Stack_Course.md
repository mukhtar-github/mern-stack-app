...
And the *address in the database is actually three different fields, we have the building which is like the number then we have the street then we have the zip code -- const address = `${restaurant.address.building} ${restaurant.address.street}, ${restaurant.address.zipcode}`.*
