...
and then we're going to be making a get request so go to uh local shows 5000 slash api slash v1 slash restaurants and now we can see all the restaurants on the side here so you can see each one that has the address it has the burrow the cuisine
