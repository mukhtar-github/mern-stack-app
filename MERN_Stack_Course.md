...
And then we're going to get a *cursor*, and then we're going to await restaurants dot find *query* so this is going to find all the restaurants from the database that go along with the *query* that we passed in if there is no *query* if we just had this blank query and it wasn't set to anything then it's just going to return all restaurants and then we're going to catch an error and then we'll just return that's an *empty* list and 0 for the total number of restaurants if there's an error but if there's no error now we're going to limit the results because it return in the *cursor* is every single result
