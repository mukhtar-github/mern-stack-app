...
So we're just making a *dummy login system*, which is basically a *form, that when someone fills out the form, they click login*, it's just going to *set the user with those details*. And it's not even going to *save the user to the database*. This is just kind of a *dummy login system* that you can easily update with a more fully full-fledged login system you can maybe even set it up to use google sign in or some sort of authentication provider.
