...
Remember, *cuisines is the list of cuisines, and we're going to map it, and for each cuisine in that array, we're going to return an option for the select box, and the value is going to be the cuisine.* And then the reason why it's *cuisine.substr(0, 20), because some cuisines are very very long, and we want to make sure the search box isn't too long.* So we're going to just have it just the first 20 characters, or is that 21 characters just the first little bit of the cuisine and so people and search the cuisine and they click the button it'll find my cuisine
