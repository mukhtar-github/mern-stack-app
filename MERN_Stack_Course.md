...
We're going to get the information from *the body, the review_id -- const reviewId = req.body.review_id*, *the text -- const text = req.body.text*. Those are the only things we need, they're just the *review_id and the text we're going to update*. We're going to automatically create a *new date -- const date = new Date()*. And then we're going to *update the review -- ReviewsDAO.updateReview*, we also need the *user_id -- req.body.user_id*, we're going to get that in the *body* too, and we just send it all over. So we get the *user_id* because we want to *make sure the user who created the review is the same one who's trying to update the review*.

and then we just check if there's an error and return the error message
