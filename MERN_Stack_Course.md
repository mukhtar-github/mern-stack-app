...
And then the first thing we're going to do is to *addReview(restaurantId, user, review, date)*, and you can see it takes a *restaurantId, user, review, and date*. And then we're going to create the *const reviewDoc = { name: user.name, user_id: user._id, date: date, text: review, restaurantId: ObjectId(restaurantId) }*, we are going to actually create a an *ObjectId* from this. Then we are going to *just insert it right into the database -- return await reviews.insertOne(reviewDoc)*, with the *restaurantId converted to a mongodb ObjectId -- restaurantId: ObjectId(restaurantId)*. And then *if there's an error it'll return the error*.
