...
So we're going to call the restaurant data service with the query and the buy if we go back to the data services we can see the query and the buy so buy would be name so let's say the name we're looking for all restaurants that have the word food and the name so by would say name and then query would say food or by could say "zipcode" -- and then query could be the number of the "zipcode"
